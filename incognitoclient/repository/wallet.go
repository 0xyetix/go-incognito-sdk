...
func NewWallet(inc *service.Incognito, constantID string, block *Block, incChainIntegration *IncChainIntegration) *Wallet {
...
func (w *Wallet) ListRewardAmounts() ([]entity.RewardAmount, error) {
	param := []interface{}{}
	resp, _, err := w.Inc.PostAndReceiveInterface(ListRewardAmount, param)
	if err != nil {
...
func (w *Wallet) GetBalanceByPaymentAddress(paymentAddress string) (uint64, error) {
	resp, _, err := w.Inc.PostAndReceiveInterface(GetBalanceByPaymentAddress, []interface{}{paymentAddress})
	if err != nil {
		return 0, err
	}
	data := resp.(map[string]interface{})
...
func (w *Wallet) GetListCustomTokenBalance(paymentAddress string) (*entity.ListCustomTokenBalance, error) {
	resp, _, err := w.Inc.PostAndReceiveInterface(GetListCustomTokenBalance, []interface{}{paymentAddress})
	if err != nil {
		return nil, err
	}
	data := resp.(map[string]interface{})
...
func (w *Wallet) GetAmountVoteToken(paymentAddress string) (*entity.ListCustomTokenBalance, error) {
	resp, _, err := w.Inc.PostAndReceiveInterface(GetAmountVoteToken, []interface{}{paymentAddress, 0})
	if err != nil {
...
func (w *Wallet) CreateAndSendConstantTransaction(privateKey string, req entity.WalletSend) (string, error) {
	param := []interface{}{privateKey, req.PaymentAddresses, constant.EstimateFee, 0}
	...
	fmt.Printf("raw data method CreateAndSendConstantTransaction: %v \n", rawData)

	resp, _, err := w.Inc.PostAndReceiveInterface(SendRawTransaction, rawData)
...
func (w *Wallet) EstimatePRVFee(privateKey, toAddress string, amountToSend uint64) (int, int, error) {
	...
	resp, _, err := w.Inc.PostAndReceiveInterface(EstimateFee, param)
	if err != nil {
...
func (w *Wallet) SendCustomTokenTransaction(privateKey string, req entity.WalletSend) (string, error) {
	param := []interface{}{privateKey, nil, constant.EstimateFee, -1}
	...
	tokenData["TokenAmount"] = 0

	param = append(param, tokenData)
	resp, _, err := w.Inc.PostAndReceiveInterface(CreateAndSendCustomTokenTransaction, param)
	if err != nil {
...
func (w *Wallet) SendPrivacyCustomTokenTransaction(privateKey string, req entity.WalletSend) (map[string]interface{}, error) {
	...
	resp, _, err := w.Inc.PostAndReceiveInterface(SendRawPrivacyCustomTokenTransaction, rawData)
...
func (w *Wallet) ListPrivacyCustomToken() ([]entity.PCustomToken, error) {
	param := []interface{}{}
	resp, _, err := w.Inc.PostAndReceiveInterface(ListPrivacyCustomToken, param)
	if err != nil {
...
func (w *Wallet) GetTxByHash(txHash string) (*entity.TransactionDetail, error) {
	param := []string{txHash}
	resp, _, err := w.Inc.PostAndReceiveInterface(GetTransactionByHash, param)
	if err != nil {
...
func (w *Wallet) GetDecryptOutputCoinByKeyOfTransaction(txHash, paymentAddress, readonlyKey string) (*entity.DecrypTransactionPRV, error) {
	...
	resp, _, err := w.Inc.PostAndReceiveInterface(DecryptOutputCoinByKeyOfTransaction, param)
	if err != nil {
		return nil, errors.Wrapf(err, "w.blockchainAPI: param: %+v", param)
	}
...
func (w *Wallet) CreateAndSendIssuingRequest(privateKey, cstDCBIssueAddress, receiveAddress string, depositedAmount *big.Int, ConstantAssetType string, ConstantCurrencyType string) (string, error) {
	if depositedAmount == nil {
		return "", errors.New("depositedAmount is nil")
	}
	receiver := map[string]uint64{
		cstDCBIssueAddress: 1,
	}
	...
	depositedReq["AssetType"] = ConstantAssetType
	depositedReq["CurrencyType"] = ConstantCurrencyType

	param := []interface{}{privateKey, receiver, constant.EstimateFee, -1, depositedReq}
...
func (w *Wallet) GetIssuingStatus(txHash string) (string, uint64, error) {
	param := []string{txHash}
	resp, _, err := w.Inc.PostAndReceiveInterface(GetIssuingStatus, param)
	if err != nil {
...
func (w *Wallet) GetContractingStatus(txHash string) (string, *big.Int, error) {
	param := []string{txHash}
	resp, _, err := w.Inc.PostAndReceiveInterface(GetContractingStatus, param)
	if err != nil {
		return "", nil, errors.Wrapf(err, "w.blockchainAPI: param: %+v", param)
	}
...
func (w *Wallet) CreateAndSendTxWithIssuingEth(privateKey, burnerAddress string, metadata map[string]interface{}) (string, []byte, error) {
	...
	resp, body, err := w.Inc.PostAndReceiveInterface(SendRawTransaction, rawData)
...
func (w *Wallet) GetBridgeReqWithStatus(TxReqID string) (int, error) {
	...
	resp, _, err := w.Inc.PostAndReceiveInterface(GetBridgeReqWithStatus, param)
	if err != nil {
...
func (w *Wallet) CreateWithDrawReward(privateKey, tokenID string) (string, error) {
	...
	resp, _, err := w.Inc.PostAndReceiveInterface(WithDrawReward, param)
	if err != nil {
...
func (w *Wallet) GenerateTokenID(symbol, pSymbol string) (string, error) {
	resp, _, err := w.Inc.PostAndReceiveInterface(GenerateTokenID, []interface{}{symbol, pSymbol})
	if err != nil {
		return "", err
	}
	data := resp.(map[string]interface{})
...
func (w *Wallet) GetPublickeyFromPaymentAddress(paymentAddress string) (string, error) {
	resp, _, err := w.Inc.PostAndReceiveInterface(GetPublickeyFromPaymentAddress, []interface{}{paymentAddress})
...
func (w *Wallet) GetShardFromPaymentAddress(paymentAddress string) (int, error) {
	...
	body, err := w.Inc.Post(GetPublickeyFromPaymentAddress, []interface{}{paymentAddress})
...
func (w *Wallet) GetBurningAddressFromChain() (string, error) {
	...
	resp, _, err := w.Inc.PostAndReceiveInterface(GetBurningAddress, param)
	if err != nil {
		return "", errors.Wrapf(err, "w.blockchainAPI: param: %+v", param)
	}
...
func (w *Wallet) GetTransactionByReceivers(PaymentAddress, ReadonlyKey string) (res *entity.ReceivedTransactions, err error) {
	...
	resp, _, err := w.Inc.PostAndReceiveInterface(GetTransactionByReceiver, param)
...
func (w *Wallet) CreateAndSendBurningForDepositToSCRequest(
	...
	if err != nil {
		return nil, errors.Wrapf(err, "w.blockchainAPI: method %+v, Get burn address", CreateAndSendBurningForDepositToSCRequest)
	}
	...
	if err != nil {
		return nil, errors.Wrapf(err, "w.blockchainAPI: method %+v", CreateAndSendBurningForDepositToSCRequest)
	}
...
func (w *Wallet) SellPRV(privateKey string, buyTokenId string, tradingFee uint64, sellTokenAmount uint64, minimumAmount uint64, traderAddress string) (string, error) {
	...
	resp, _, err := w.Inc.PostAndReceiveInterface(CreateAndSendTxWithPRVTradeReq, paramArray)
	if err != nil {
...
func (w *Wallet) SellPToken(privateKey string, buyTokenId string, tradingFee uint64, sellTokenId string, sellTokenAmount uint64, minimumAmount uint64, traderAddress string, networkFeeTokenID string, networkFee uint64) (string, error) {
	...
	resp, _, err := w.Inc.PostAndReceiveInterface(CreateAndSendTxWithPTokenTradeReq, paramArray)
	if err != nil {
		return "", errors.Wrapf(err, "w.blockchainAPI")
	}
...
func (w *Wallet) SellPRVCrosspool(privateKey string, buyTokenId string, tradingFee uint64, sellTokenAmount uint64, minimumAmount uint64, traderAddress string) (string, error) {
	var burningAddress string
	var err error
	burningAddress, err = w.Block.GetBurningAddress()
	...
	resp, _, err := w.Inc.PostAndReceiveInterface(CreateAndSendTxWithPRVCrosspollTradeReq, paramArray)
	if err != nil {
...
func (w *Wallet) SellPTokenCrosspool(privateKey string, buyTokenId string, tradingFee uint64, sellTokenId string, sellTokenAmount uint64, minimumAmount uint64, traderAddress string, networkFeeTokenID string, networkFee uint64) (string, error) {
	var burningAddress string
	var err error
	burningAddress, err = w.Block.GetBurningAddress()
	var FeePerKb int
	...
	resp, _, err := w.Inc.PostAndReceiveInterface(CreateAndSendTxWithPTokenCrosspolTradeReq, paramArray)
	if err != nil {
		return "", errors.Wrapf(err, "w.blockchainAPI: param: %+v", paramArray)
	}
