...
func NewBlock(inc *service.Incognito) *Block {
...
func (b *Block) GetBlockInfo(blockHeight int32, shardID int) (*entity.GetBlockInfo, error) {
	...
	resp, _, err := b.Inc.PostAndReceiveInterface(retrieveblockbyheight, param)
...
func (b *Block) GetBlockChainInfo() (*entity.GetBlockChainInfoResult, error) {
	param := []interface{}{}
	resp, _, err := b.Inc.PostAndReceiveInterface(getblockchaininfo, param)
...
func (b *Block) GetBestBlockHeight(shardID int) (uint64, error) {
	...
	resp, _, err := b.Inc.PostAndReceiveInterface(GetBlockCount, param)
	if err != nil {
...
// GetSwapProof calls rpc api to Incognito chain and returns proof.GetInstructionProof object
func (b *Block) GetSwapProof(
	blockHeight uint64,
	rpcFuncName string,
) (*ethereum.GetInstructionProof, error) {
	param := []interface{}{blockHeight}
	resp, _, err := b.Inc.PostAndReceiveInterface(rpcFuncName, param)
	if err != nil {
		return nil, errors.Wrapf(err, "b.blockchainAPI: param: %+v", param)
	}
	data, ok := resp.(map[string]interface{})
	if !ok {
		return nil, errors.Errorf("Wrong response format, it should be an object that contains Error or Result field")
	}
	if data["Error"] != nil {
		errContent := data["Error"].(map[string]interface{})
		statusCode := int(errContent["Code"].(float64))
		if statusCode == -7000 {
			return nil, nil
		}
		return nil, errors.Errorf("couldn't get result from response data: %+v", data["Error"])
	...
	resBytes, _ := json.Marshal(data["Result"])
	var getInstructionProof ethereum.GetInstructionProof
	err = json.Unmarshal(resBytes, &getInstructionProof)
	if err != nil {
		return nil, errors.Errorf("Wrong response format, the returned object type should be *bridge.GetInstructionProof.")
	}
	return &getInstructionProof, nil
}
...
func (b *Block) GetBeaconBestStateDetail() (res *entity.BeaconBestStateResp, err error) {
	empty := []string{}
	body, err := b.Inc.Post(GetBeaconBestStateDetail, empty)
	if err != nil {
		return nil, err
	}
...
func (b *Block) GetBurningAddress() (string, error) {
	...
	resp, _, err := b.Inc.PostAndReceiveInterface(GetBurningAddress, paramArray)
	if err != nil {
