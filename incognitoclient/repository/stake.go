...
func NewStake(inc *service.Incognito, incChainIntegration *IncChainIntegration) *Stake {
	return &Stake{Inc: inc, IncChainIntegration: incChainIntegration}
}

func (s *Stake) ListUnstake() ([]entity.Unstake, error) {
	param := []interface{}{}
	resp, _, err := s.Inc.PostAndReceiveInterface(GetBeaconBestStateDetail, param)
	if err != nil {
...
func (s *Stake) GetTotalStaker() (float64, error) {
	param := []interface{}{}
	resp, _, err := s.Inc.PostAndReceiveInterface(GetTotalStaker, param)
	if err != nil {
...
func (b *Stake) CreateAndSendStakingTransaction(receiveRewardAddress, privateKey, userPaymentAddress, userValidatorKey, burnTokenAddress string) (string, error) {
	...
	resp, _, err := b.Inc.PostAndReceiveInterface(SendRawTransaction, rawData)
	if err != nil {
...
func (b *Stake) CreateAndSendUnStakingTransaction(privateKey, userPaymentAddress, userValidatorKey, burnTokenAddress string) (string, error) {
	param := []interface{}{
		privateKey,
...
func (b *Stake) CreateWithDrawReward(privateKey, paymentAddress, tokenID string) (string, error) {
	param := []interface{}{
		privateKey,
		nil,
		0,
...
func (b *Stake) GetRewardAmount(paymentAddress string) ([]entity.RewardItems, error) {
	...
	resp, _, err := b.Inc.PostAndReceiveInterface(RewardAmount, param)
	if err != nil {
...
func (b *Stake) GetNodeAvailable(validatorKey string) (float64, error) {
	...
	resp, _, err := b.Inc.PostAndReceiveInterface(RoleByValidatorKey, param)
	if err != nil {
